 ...
 * system differences. A generic
 * state change is not directly translatable on Windows. Therefore,
 * PROCESS_STATE_CHANGE will only occur on
 * POSIX systems.
 */
enum ProcessState {
  PROCESS_ERROR = -1,
...
class PlatformProcess : private boost::noncopyable {
  ...
  * extension name will be stripped
  * away.
  */
